...
export default function DoctorForm({ doctor, closeForm }) {
    ...
    async function saveDoctor(doctor) {
        try {
            ...
            let response;
            if (!doctor._id) {
                response = await axios.post('http://localhost:3000/doctor/create', formDoctor);
            } else {
                response = await axios.put('http://localhost:3000/doctor/update/' + doctor._id, formDoctor);
            }
